...
import {
  ...
  DnsRounded as Network,
  GppGoodRounded as ShieldCheck,
  ...
  return (
    <div style={{ display: 'flex', height: '100%', width: '100%', overflow: 'hidden' }}>
      ...
      <section style={{ flex: 1, padding: 32, display: 'flex', flexDirection: 'column', gap: 24, borderRight: '1px solid var(--border)', background: 'var(--bg-card)' }}>
        ...
        {error && (
          <div style={{ padding: '12px 16px', borderRadius: 0, background: 'var(--error-bg)', border: '1px solid var(--error)', fontSize: 13, color: 'var(--error)', display: 'flex', gap: 10, alignItems: 'center' }}>
        ...
        <label style={{
          ...
          {!preview && (
            <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', opacity: 0.2 }}>
              <Database sx={{ fontSize: 64, marginBottom: 2 }} />
          ...
          {preview && (
            <>
              ...
              <div style={{ position: 'absolute', bottom: 24, left: 24, right: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                <div>
                  <p className="text-label" style={{ color: 'var(--text-muted)', fontFamily: 'monospace' }}>SOURCE HASH: VERIFIED</p>
                  <p className="font-headline" style={{ fontSize: 24, fontWeight: 800 }}>{file?.name}</p>
                </div>
                <div style={{ background: 'var(--accent-container)', padding: '4px 12px', borderRadius: 0, border: '1px solid var(--border)' }}>
                  <span className="text-label" style={{ color: 'var(--text-primary)' }}>AUTHENTICATED</span>
                </div>
              </div>
            </>
          )}
      ...
      <section style={{ width: 450, padding: 32, display: 'flex', flexDirection: 'column', gap: 32, flexShrink: 0, background: 'var(--bg-primary)' }}>
        ...
        <div className="panel" style={{ padding: 24, boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <span className="font-headline text-label" style={{ fontWeight: 800 }}>Partition Status</span>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div className="hover-target" style={{ aspectRatio: '1/1', background: 'var(--bg-card)', border: '1px solid var(--border)', padding: 16, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', cursor: 'pointer', borderRadius: 0 }}>
              <HardDrive sx={{ fontSize: 24, color: 'var(--text-muted)' }} />
              <p className="text-label" style={{ fontSize: 11 }}>Local Storage</p>
            </div>
            <div className="hover-target" style={{ aspectRatio: '1/1', background: 'var(--bg-card)', border: '1px solid var(--border)', padding: 16, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', cursor: 'pointer', borderRadius: 0 }}>
              <Network sx={{ fontSize: 24, color: 'var(--text-muted)' }} />
              <p className="text-label" style={{ fontSize: 11 }}>Peer Network</p>
            </div>
          </div>
        </div>
        ...
        {shares.length > 0 && (
          <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 12, flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 16, height: 100 }}>
              {shares.map((s, i) => (
                <div key={i} onClick={() => dl(s, `share_${i + 1}.png`)} className="hover-target" style={{ flex: 1, background: 'var(--bg-card-highest)', borderRadius: 0, border: '1px solid var(--border)', overflow: 'hidden', cursor: 'pointer', position: 'relative' }}>
                  ...
                  <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, transition: 'opacity 0.2s' }} onMouseEnter={e => e.currentTarget.style.opacity = '1'} onMouseLeave={e => e.currentTarget.style.opacity = '0'}>
                    <Download sx={{ color: 'var(--accent-primary)' }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Final Action */}
        <button className="btn-primary" onClick={split} disabled={!file || loading} style={{ width: '100%', height: 64 }}>
          Generate Partitioned Shares
        </button>
